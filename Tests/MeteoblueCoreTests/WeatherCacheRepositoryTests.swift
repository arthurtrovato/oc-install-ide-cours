...
final class WeatherCacheRepositoryTests: XCTestCase {
    ...
    func testFreshCacheAvoidsNetwork() async throws {
        let snapshot = SampleWeatherFactory.make(fetchedAt: now.addingTimeInterval(-60 * 60))
    ...
    func testNewPositionForcesNetworkEvenWhenOldCacheFresh() async throws {
        let old = SampleWeatherFactory.make(fetchedAt: now)
        ...
        let paris = WeatherSnapshot(
            fetchedAt: now,
            location: parisLocation,
            current: old.current,
            hourly: old.hourly,
            daily: old.daily,
            meteoblueURL: try! MeteoblueForecastLinkBuilder.webURL(for: parisLocation)
        )
        let counter = CountingWeatherService.Counter()
        let result = try await WeatherRepository(
            service: CountingWeatherService(counter: counter, result: .success(paris)),
            cache: InMemoryWeatherCacheStore(records: [WeatherCacheRecord(snapshot: old)])
        ).weather(for: parisLocation, at: now)
        XCTAssertEqual(result.source, .network)
        XCTAssertEqual(counter.read(), 1)
        XCTAssertEqual(result.snapshot.location.locality, "Paris")
    }

    func testOfflineAtNewPositionFallsBackToLastKnownData() async throws {
        let old = SampleWeatherFactory.make(fetchedAt: now.addingTimeInterval(-30 * 60))
        let parisLocation = WeatherLocation(
            coordinate: GeoCoordinate(latitude: 48.8566, longitude: 2.3522),
            locality: "Paris",
            countryCode: "FR",
    ...
    func testReturnToPreviousZoneReusesItsFreshCache() async throws {
        let tressange = SampleWeatherFactory.make(fetchedAt: now)
        ...
        let paris = WeatherSnapshot(fetchedAt: now, location: parisLocation, current: tressange.current, hourly: tressange.hourly, daily: tressange.daily, meteoblueURL: try! MeteoblueForecastLinkBuilder.webURL(for: parisLocation))
        ...
        let result = try await WeatherRepository(
            ...
            cache: cache
        ).weather(for: tressange.location, at: now.addingTimeInterval(10 * 60))
        XCTAssertEqual(result.source, .freshCache)
